...
public abstract class AbstractCodePoolExecutorService implements CodePoolExecutorService {
	...
	protected final ConcurrentHashMap<String, State> coreContainerState;

	// Record temporary container status
	protected final ConcurrentHashMap<String, State> tempContainerState;
	...
	protected final ArrayBlockingQueue<FutureTask<TaskResponse>> taskQueue;
	...
	private TaskResponse pushTaskQueue(TaskRequest request) throws ExecutionException, InterruptedException {
		FutureTask<TaskResponse> ft = new FutureTask<>(() -> {
	...
	private void popTaskQueue() {
		FutureTask<TaskResponse> future = this.taskQueue.poll();
		if (future == null) {
			return;
		}
